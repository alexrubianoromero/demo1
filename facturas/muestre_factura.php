...
$sql_muestre_ordenes = "select f.numero_factura,f.fecha,f.placa,f.id_factura,e.prefijo_factura,f.tipo_factura,f.anulado,f.resolucion 
from $tabla11 as f
...
<?php
echo '<table border= "1">';
	echo '<td><h3>No FACTURA<h3></td><td><h3>TIPO FACTURA<h3></td><td><h3>ANULADO<h3></td><td><h3>Fecha</h3></td><td><h3>Placa/Identidad</h3></td><td><h3>modificar</h3></td>'; 
	echo '<td><h3>Enviar_Correo<h3></td>';
		while($ordenes = mysql_fetch_array($consulta_ordenes))
			{ 
				...
				echo '<td><h3>'.$ordenes['0'].'</h3></td><td><h3>'.$documento.'</h3></td>';
				echo '<td><h3>'.$ordenes['6'].'</h3></td>';
				echo'<td><h3>'.$ordenes['1'].'</h3></td><td><h3>'.$identificacion.'</h3></td>';
